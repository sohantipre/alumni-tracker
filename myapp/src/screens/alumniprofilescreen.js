...
const AlumniProfilescreen = (props) => {
    ...
    axios
      ...
      .then((value) => {
        setinfo({
          ...
          loading: false,
          followerlist: value.data.followerlist,
          followinglist: value.data.followinglist,
        });

        console.log(value.data.followerlist);
        console.log(id);
        var s = value.data.followerlist.some((f) => f._id == id);
        console.log("s", s);
        if (s) {
          setfollow(true);
        }
  ...
  const handlefollow = () => {
    axios
      .post(`/student/follow/${props.match.params.id}`, info)
      .then((value) => {
        setfollow(true);
  ...
  const handleunfollow = () => {
    axios
      .post(`/student/unfollow/${props.match.params.id}`, info)
      .then((value) => {
        setfollow(false);
        window.location.reload();
        alert("Unfollowed");
    ...
    <div>
      {alumninfo.loading ? (
        ...
        <div style={{ backgroundColor: "#ebe0e0", bottom: "0px" }}>
          ...
          <h2>followers :{alumninfo.followerlist.length} </h2>
          <h2>following:{alumninfo.followinglist.length}</h2>
