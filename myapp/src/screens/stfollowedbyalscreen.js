...
const Followedscreen = (props) => {
  const [studentinfo, setstate] = useState({
    students: [],
    error: "",
    loading: true,
  });
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:5000/alumni/getfollowedstudents/${props.match.params.id}`
      )
      .then((value) => {
        setstate({ loading: false, students: value.data.students });
  ...
  //           ></img> */}
  //           <h1>Student List</h1>
  //           {studentinfo.students.length == 0 ? (
  //             <h3 style={{ marginTop: "20px" }}>
  //               You haven't followed any student yet!!
  //             </h3>
  //           ) : (
  //             <></>
  //           )}
  //           {studentinfo.students.map((a, i) => (
  //             <h2>
  //               <a
  //                 style={{ textDecoration: "none", color: "black" }}
  //                 href={`/studentprofile/${a.id}`}
  //                 // onClick={() => {
  ...
  return (
    <div>
      {studentinfo.loading ? (
        ...
        <div className='collegescreen' style={{ fontSize: "10px" }}>
          <h1 style={{ marginTop: "20px" }}>Student List</h1>
          {studentinfo.students.length == 0 ? (
            <>You haven't followed any student yet!!!</>
          ) : (
            <></>
