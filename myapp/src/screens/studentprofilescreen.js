...
const Studentprofilescreen = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5000/student/studentprofile/${props.match.params.id}`
      )
      .then((value) => {
        setinfo({
  ...
  const handlefollow = () => {
    axios
      .post(`/alumni/follow/${props.match.params.id}`, info)
      .then((value) => {
        setfollow(true);
  ...
  const handleunfollow = () => {
    axios
      .post(`/alumni/unfollow/${props.match.params.id}`, info)
      .then((value) => {
        setfollow(false);
        window.location.reload();
        alert("Unfollowed");
